...
## Zerops Challenge delivery boundary

- This existing VibeMarketer codebase is the product to adapt; do not rebuild it or replace `packages/engine` without evidence that the current path cannot support the requirement.
- The Zerops work must make the existing URL → brand memory → channel drafts → human approval → provider-confirmed publish → report flow more reliable, not add infrastructure for its own sake.
- Use the project-local `zerops` MCP server from `.mcp.json` and the installed `zcp`/`zcli` tools for Zerops operations. Inspect current state before any service, env, deployment, or migration mutation.
- Keep Supabase Auth if replacing it would risk the core flow. Move persistence or queue work only when the existing code and tests show a clear boundary.
- Never fake provider results, publication IDs, research, model output, queue completion, or deployment health. A missing credential or unavailable provider must remain an explicit failure.
- Keep `docs/ZEROPS-HACK-PLAN.md` and architecture/acceptance documentation reviewable. Do not create application code until the audit plan identifies the exact P0 slice.
- No production deployment, external publication, GitHub push, or irreversible migration is authorized merely by this setup task.
