...
import { isZeropsPostgresConfigured } from "@/lib/zerops-postgres";
import { getSupabaseAdmin, hasSupabaseAdmin } from "@/lib/supabase-admin";
import { ZeropsPublishAttemptRepository } from "./zerops-publish-attempt-repo";
import type {
  PublishAttempt,
...
export class SupabasePublishAttemptRepository {
...
type PublishAttemptRepositoryDelegate =
  | SupabasePublishAttemptRepository
  | ZeropsPublishAttemptRepository;

function shouldUseZeropsPublishingRepository(): boolean {
  const forced = process.env.MARKETING_STORE_BACKEND?.trim().toLowerCase();
  if (forced === "zerops") return isZeropsPostgresConfigured();
...
/**
 * Select the durable publishing repository for the current deployment.
 * Zerops uses its managed PostgreSQL tables; other environments retain the
 * existing Supabase repository until they opt into the migration.
 */
export class PublishAttemptRepository {
  private readonly delegate: PublishAttemptRepositoryDelegate;

  constructor() {
    this.delegate = shouldUseZeropsPublishingRepository()
      ? new ZeropsPublishAttemptRepository()
      : new SupabasePublishAttemptRepository();
  }

  getPublishAttemptById(ownerId: string, attemptId: string) {
    return this.delegate.getPublishAttemptById(ownerId, attemptId);
  }

  getPublishAttemptByIdempotencyKey(ownerId: string, idempotencyKey: string) {
    return this.delegate.getPublishAttemptByIdempotencyKey(ownerId, idempotencyKey);
  }

  getOutboxJobByAttempt(attemptId: string) {
    return this.delegate.getOutboxJobByAttempt(attemptId);
  }

  createOutboxJobIfMissing(
    ownerId: string,
    attemptId: string,
    jobType: PublishOutboxJobType = "publish",
  ) {
    return this.delegate.createOutboxJobIfMissing(ownerId, attemptId, jobType);
  }
  ...
  createOrReusePublishAttempt(input: {
    ...
    return this.delegate.createOrReusePublishAttempt(input);
  }

  claimNextOutboxJob(leaseOwner: string, leaseMs?: number) {
    return this.delegate.claimNextOutboxJob(leaseOwner, leaseMs);
  }

  renewOutboxLease(jobId: string, leaseOwner: string, leaseMs?: number) {
    return this.delegate.renewOutboxLease(jobId, leaseOwner, leaseMs);
  }

  releaseExpiredLeases() {
    return this.delegate.releaseExpiredLeases();
  }

  markAttemptExecuting(attemptId: string) {
    return this.delegate.markAttemptExecuting(attemptId);
  }
  ...
  markProviderSucceeded(
    ...
    return this.delegate.markProviderSucceeded(attemptId, patch);
  }
  ...
  markOutcomeUnknown(
    ...
    return this.delegate.markOutcomeUnknown(attemptId, opts);
  }
  ...
  markRetryableFailure(
    ...
    return this.delegate.markRetryableFailure(attemptId, opts);
  }
  ...
  markPermanentFailure(
    ...
    return this.delegate.markPermanentFailure(attemptId, opts);
  }
  ...
  markAttemptPublished(
    ...
    return this.delegate.markAttemptPublished(attemptId, providerPostId, providerUrl);
  }

  markAttemptCancelled(attemptId: string) {
    return this.delegate.markAttemptCancelled(attemptId);
  }

  completeOutboxJob(jobId: string) {
    return this.delegate.completeOutboxJob(jobId);
  }
  ...
  rescheduleOutboxJob(
    ...
    return this.delegate.rescheduleOutboxJob(jobId, options);
  }
  ...
  deadLetterOutboxJob(
    ...
    return this.delegate.deadLetterOutboxJob(jobId, opts);
  }

  cancelOutboxJob(jobId: string) {
    return this.delegate.cancelOutboxJob(jobId);
  }

  listAttemptsForOwner(ownerId: string) {
    return this.delegate.listAttemptsForOwner(ownerId);
  }
}
