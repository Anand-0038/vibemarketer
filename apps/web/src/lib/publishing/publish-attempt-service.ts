...
import { publishPublishWakeup } from "./nats-wakeup";
...
> {
  ...
  const repo = deps.repo ?? new PublishAttemptRepository();
  const { attempt, outboxJob } = await createPublishAttemptForPost(
    {
      ownerId,
      post: {
        ...queued,
  ...
  const wakeup = await publishPublishWakeup({
    attemptId: attempt.id,
    jobId: outboxJob.id,
    ownerId,
  });
  if (!wakeup.queued) {
    (deps.logger ?? logEvent)("publish_queue_wakeup_unavailable", {
