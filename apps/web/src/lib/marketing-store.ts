 ...
 * - Zerops production: direct PostgreSQL is the source of truth for marketing state.
 * - Other production hosts: Supabase `marketing_state` remains the source of truth.
 ...
 */
export type MarketingBackend = "zerops" | "supabase" | "local";

export function resolveMarketingBackend(): MarketingBackend {
  ...
  if (isZerops) {
    if (!isZeropsPostgresConfigured()) {
      throw new MarketingStoreError(
        "Zerops production requires DATABASE_URL for marketing persistence",
        "MISCONFIGURED",
        500,
      );
    }
...
export class MarketingStore {
  ...
  private backend(): MarketingBackend {
    return resolveMarketingBackend();
  }

  private requireOwnerForCloud(): void {
    if (this.backend() !== "local" && this.ownerId === "anonymous") {
      throw new MarketingStoreError(
  ...
  async load(): Promise<MarketingData> {
    ...
    if (backend === "zerops") {
      try {
        ...
        const result = await getZeropsPool().query<{
          data: Partial<MarketingData>;
          version: number;
        }>(
          `select data, version
             from public.vibemarketer_marketing_state
            where owner_id = $1`,
          [this.ownerId],
        );
        ...
        this.rowVersion = Number(row.version);
        this.data = coerceMarketingData(row.data);
        this.data.version = this.rowVersion;
        this.loaded = true;
        return this.data;
      } catch (error) {
        throw new MarketingStoreError(
          `Zerops marketing state unavailable: ${
  ...
  async save(): Promise<void> {
    ...
    if (backend === "zerops") {
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        try {
          if (expected === null) {
            const inserted = await getZeropsPool().query<{ version: number }>(
              `insert into public.vibemarketer_marketing_state
                 (owner_id, data, version, updated_at)
               values ($1, $2::jsonb, $3, $4)
               returning version`,
              [this.ownerId, JSON.stringify(payload), nextVersion, updatedAt],
            );
            ...
            const updated = await getZeropsPool().query<{ version: number }>(
              `update public.vibemarketer_marketing_state
                  set data = $1::jsonb, version = $2, updated_at = $3
                where owner_id = $4 and version = $5
                returning version`,
              [
          ...
          if (expected === null && code === "23505") {
            await this.load();
            continue;
          }
          throw new MarketingStoreError(
            `Marketing state save failed: ${
              error instanceof Error ? error.message : "database error"
            }`,
            "UNAVAILABLE",
            503,
